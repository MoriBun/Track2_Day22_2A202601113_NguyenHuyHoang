...
# RAGAS 0.4.x imports this legacy integration even when evaluation uses OpenAI.
# Newer LangChain Community releases no longer ship it, so provide a harmless
# compatibility type; it is never instantiated in this OpenAI-based lab.
try:
    from langchain_community.chat_models.vertexai import ChatVertexAI  # noqa: F401
except ModuleNotFoundError:
    vertexai_module = ModuleType("langchain_community.chat_models.vertexai")
...
SYSTEM_V1 = (
    "You are a friendly AI assistant. Use only the context below to answer the "
    "question. Keep the response clear and concise, in 2-4 sentences. If the "
    "context does not contain the answer, say so directly.\n\nContext:\n{context}"
)
...
SYSTEM_V2 = (
    "You are an AI analysis expert. Use only the context below to answer. Give a "
    "structured response in 3-5 sentences: state the main conclusion, explain the "
    "relevant facts, and explicitly say when the context is insufficient. Do not "
